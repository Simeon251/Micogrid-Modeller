...
with st.sidebar:
    ...
    num_days = setup_col2.slider("Simulation duration (days)", min_value=1, max_value=365, value=365, help="How many days the simulator will model before annualizing the results for lifecycle economics.")
    ...
    pv_capacity_kwp = pv_col1.number_input("Array capacity (kWp)", min_value=0.0, value=180.0, step=10.0, help="Installed DC PV array size at standard test conditions.")
    ...
    wind_capacity_kw = wind_col1.number_input("Rated power (kW)", min_value=0.0, value=30.0, step=10.0, help="Nameplate AC output of the wind turbine.")
    ...
    diesel_use_kva = st.toggle("Input diesel in kVA", value=False)
    if diesel_use_kva:
        diesel_capacity_kva = st.number_input("Generator rating (kVA)", min_value=0.0, value=100.0, step=5.0, help="Apparent power rating from the generator datasheet.")
        diesel_capacity_kw = 0.0
    else:
        diesel_capacity_kw = st.number_input("Prime rating (kW)", min_value=0.0, value=80.0, step=5.0, help="Real power rating if your datasheet already gives the generator in kW.")
    ...
    fuel_25 = fuel_col1.number_input("Fuel use at 25% load (L/h)", min_value=0.0, value=7.5, step=0.1, help="Fuel use from the generator datasheet at 25% loading.")
    fuel_50 = fuel_col2.number_input("Fuel use at 50% load (L/h)", min_value=0.0, value=12.3, step=0.1, help="Fuel use from the generator datasheet at 50% loading.")
    fuel_75 = fuel_col1.number_input("Fuel use at 75% load (L/h)", min_value=0.0, value=18.3, step=0.1, help="Fuel use from the generator datasheet at 75% loading.")
    fuel_100 = fuel_col2.number_input("Fuel use at 100% load (L/h)", min_value=0.0, value=24.5, step=0.1, help="Fuel use from the generator datasheet at full load.")
    ...
    battery_capacity_kwh = batt_col1.number_input("Energy capacity (kWh)", min_value=0.0, value=220.0, step=10.0, help="Total usable battery energy capacity.")
    battery_power_kw = batt_col2.number_input("Power capacity (kW)", min_value=0.0, value=70.0, step=5.0, help="Maximum battery charge or discharge power.")
    ...
    battery_charge_eff = st.slider("Charge efficiency", min_value=0.5, max_value=1.0, value=0.94, step=0.01, help="Fraction of charging energy that is stored in the battery.")
    battery_discharge_eff = st.slider("Discharge efficiency", min_value=0.5, max_value=1.0, value=0.94, step=0.01, help="Fraction of stored energy that can be delivered back to the system.")
    ...
    battery_degradation_temp_sensitivity = st.number_input("Battery degradation temp sensitivity", min_value=0.0, value=0.010, step=0.005, format="%.3f", help="Multiplier that increases battery fade above 25 C.")
    battery_eol_fraction = st.slider("Battery end-of-life fraction", min_value=0.50, max_value=0.95, value=0.70, step=0.01, help="Battery replacement threshold as remaining capacity fraction.")
    ...
    fuel_price_per_liter = econ_col1.number_input("Fuel price ($/L)", min_value=0.0, value=1.35, step=0.05, help="Current diesel fuel price used in dispatch and lifecycle economics.")
    ...
    energy_tariff_per_kwh = econ_col2.number_input("Energy tariff ($/kWh)", min_value=0.0, value=0.90, step=0.01, help="Average revenue tariff used for DSCR and project cash flow.")
    ...
    with st.expander("Advanced: O&M, Debt and Risk", expanded=False):
        ...
        battery_fixed_om_per_kwh_year = om_col2.number_input("Battery fixed O&M ($/kWh-yr)", min_value=0.0, value=4.0, step=1.0, help="Annual fixed operations and maintenance cost per kWh of installed battery energy.")
        ...
        battery_variable_om_per_kwh = om_col2.number_input("Battery variable O&M ($/kWh)", min_value=0.0, value=0.00, step=0.01, format="%.2f", help="Variable cost applied to each kWh discharged from the battery.")
